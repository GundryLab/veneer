...
#    useShinyjs(),        
    tags$img(src="Veneer_Overview.png", width="583px", align="right"),
    ...
    p("Veneer was written in Python and R and the web application was developed using the Shiny library. Source 
       code and all reference lookup tables are publicly available at GitHub.")
...
#    p(tags$i("Before you begin:")),
    ...
    p("It is strongly recommended that all users read the User Guide which contains 
       step-by-step tutorials for each. The User Guide comprehensively defines the 
       functions and annotations provided in Veneer."
    ),
    p(style="font-size: 17px", tags$i("Example files:")),
    p("Import file format is critical to success. File form is .xlsx. The two required 
       column headers are “Master Protein Accessions” and “Annotated Sequence”. 
       Peptide sequences must be annotated such that [K.peptidesequenceR.] Amino 
       acid modifications are indicated by lowercase letter. Example: N for unmodified 
       asparagine, n or modified asparagine (i.e. deamidated)."
    ),
    ...
    p("Veneer operates with Uniprot accession identifiers. Bulk conversion of alternate 
       ...
       available on the Uniprot website, found here. Note that conversion between IDs 
       is not always one-to-one. Manual curation of the results from the ID mapping is 
       advisable."
    ),
    p(style="font-size: 17px", tags$i("Species availability:")),
    p("Currently, annotations are available for human, mouse, and rat data. If you 
